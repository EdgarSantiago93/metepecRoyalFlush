...
export type AppState =
  ...
  | { status: 'season_active'; season: Season; members: SeasonMember[]; session: Session | null; participants: SessionParticipant[]; injections: SessionInjection[]; endingSubmissions: EndingSubmission[]; users: User[] }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!season) {
        ...
        // Fetch participants when session is in dealing, in_progress, or closing state
        let participants: SessionParticipant[] = [];
        let injections: SessionInjection[] = [];
        let endingSubmissions: EndingSubmission[] = [];
        if (session && (session.state === 'dealing' || session.state === 'in_progress' || session.state === 'closing')) {
          const partRes = await api.getSessionParticipants(session.id);
          participants = partRes.participants;
        }
        if (session && (session.state === 'in_progress' || session.state === 'closing')) {
        ...
        setState({ status: 'season_active', season, members, session, participants, injections, endingSubmissions, users });
      } else {
